                        ...
                        <div class="modal-dialog" style="min-width: 80%">
                            <div class="modal-content">
                                ...
                                <div class="modal-header">
                                    <h5 class="modal-title">Score: <%= name%> </h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                </div>
                                <div class="modal-body">
                                    <table style="width: 100%">
                                        <tr><td>Title</td><td>Answer</td><td>Status</td><td>Score</td></tr>
                                        <% List<DoDTO> listScore = (List<DoDTO>) request.getAttribute("listScore");
                                        float sum =0;
                                            if (listScore.size() > 0) {
                                                for (DoDTO item : listScore) {
                                                    sum+=item.getScore();
                                        %>
                                        <tr>
                                            <td><%= item.getExercise().getTitle()%></td>
                                            <td><%= item.getSolution()%></td>
                                            <% if (item.getSolution().isEmpty()) {
                                            %>
                                            <td>Not Answer</td>
                                            <%} else {
                                                if (item.isIsGrade()) {%>
                                        ...
                                        <% }
                                            ...
                                            <tr style="color: red;font-size: 20px"><td colspan="3" style="text-align: center">Average Score:</td><td><%= (float)sum/listScore.size() %></td></tr>
                                    </table>
                                </div>
