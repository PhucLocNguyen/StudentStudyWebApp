...
public class AnswerQuestion extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equals("score")) {
            ...
            for (DoDTO doDTO : listScore) {
                System.out.println("score: " + doDTO.getScore());
            }
            if (listScore.size() > 0) {
            ...
            System.out.println("exercise name: " + exercise.getTitle());
            request.setAttribute("exercise", exercise);

            boolean checkTime = false;
            Timestamp timeNow = new Timestamp(System.currentTimeMillis());
            Timestamp endDate = exercise.getEndDate();
            if (endDate != null) {
                if (timeNow.after(endDate)) {
                    checkTime = false;
                } else {
                    checkTime = true;
                }
            ...
            if (action.equals("") || action.equals("answer")) {
                ...
                request.setAttribute("checkAnswer", checkAnswer);
                ...
                listDo = doDAO.list(exercise_id);
                request.setAttribute("checkAnswer", false);
                request.setAttribute("listDo", listDo);
                request.getRequestDispatcher("answerQuestion.jsp").forward(request, response);
            } else if (action.equals("delete")) {
                doDAO.delete(exercise_id, student_id);
                request.setAttribute("checkAnswer", true);
                request.getRequestDispatcher("answerQuestion.jsp").forward(request, response);
            }
        }
