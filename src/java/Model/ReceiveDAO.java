...
public class ReceiveDAO {
    ...
    public boolean seenTheNotification(int notificationID, int studentID) {
        ...
        try {
            ...
            System.out.println("SQL ERROR Update seen Receive For Student In Class in ReceiveDAO: " + e.getMessage());
            e.getStackTrace();
        }
        return output;
    }

    public boolean deleteTheNotification(int notificationID, int studentID) {
        boolean output = false;
        Connection con = null;
        PreparedStatement preStm = null;
        String sql;
        ...
        try {
            ...
            sql = "DELETE FROM [dbo].[Receive] WHERE student_id = ? AND notification_id = ?";
            preStm = con.prepareStatement(sql);
            preStm.setInt(1, studentID);
            preStm.setInt(2, notificationID);
            if (preStm.executeUpdate() > 0) {
            ...
            System.out.println("SQL ERROR Delete Receive For Student In Class in ReceiveDAO: " + e.getMessage());
            e.getStackTrace();
        }
        return output;
    }
